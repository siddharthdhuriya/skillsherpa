 ...
 * Run) to schedule syncCourseSheet at ~8:00 and ~20:00 in the script's
 * timezone (Project Settings -> General -> Time zone controls which zone
 * "atHour" uses). Re-running it is safe — it clears any triggers this
 * script already owns before creating fresh ones, so it won't pile up
...
function setupTriggers() {
  ...
  Logger.log("Triggers created: daily around 8:00 and 20:00.");
}

/**
 * SETUP NOTES
 * 1. Extensions -> Apps Script, delete the placeholder code, paste this file.
 * 2. Update SYNC_URL above if your domain differs.
 ...
 *    `syncCourseSheet` in the dropdown and click Run.
